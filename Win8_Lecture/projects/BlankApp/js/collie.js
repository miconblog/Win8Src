 ...
 * });
 * @example
 * // 특정 이름을 사용 가능하게 만들어 놓고 나중에 이미지를 로딩
 * var list = {
 * 	image1 : "image1.png",
 * 	image2 : "image2.png"
 * };
 * 
 * // 마킹하면 해당 이름을 미리 사용할 수 있다
 * collie.ImageManager.mark(list);
 * 
 * // addImages 전에도 이미지를 사용 가능
 * new collie.DisplayObject({
 * 	backgroundImage : "image1"
 * });
 * 
 * // 나중에 이미지를 로딩함
 * collie.ImageManager.addImages(list);
 * @namespace
 */
collie.ImageManager = new (collie.Class(/** @lends collie.ImageManager */{
	...
	addImages : function (htList, fCallback, fFail) {
		...
		// 로드
		for (var i in htList) {
			this.load(i, htList[i], fOnComplete, fOnFail);
		}
	},
	
	/**
	 * 비동기로 이미지를 로딩
	 * TODO 비동기라서 complete 되지 않았는데 나오지는 않을지 상황을 여러개 조합해서 테스트 해봐야 함
	 * 
	 ...
	 */
	load : function (sName, sURL, fCallback, fFail) {
 ...
 * @param {String} [htOption.visibility="visible"] 화면 표시 여부 [visible|hidden], CSS의 visibility와 같다
...
collie.Layer = collie.Class(/** @lends collie.Layer.prototype */{
	...
	$init : function (htOption) {
		...
		this.optionSetter("visibility", this._setVisibility.bind(this)); // 처음 set은 Drawing이 생성된 후에 실행 된다
	...
	/**
	 * CSS의 visibility 속성과 유사
	 * @private
	 */
	_setVisibility : function () {
		if (this.getElement()) {
			this.getElement().style.visibility = this.option("visibility");
		}
	},
	 ...
	 * @param {collie.DisplayObject} oDisplayObject
	 */
	removeChild : function (oDisplayObject) {
...
collie.DisplayObjectCanvas = collie.Class(/** @lends collie.DisplayObjectCanvas.prototype */{
	...
	drawImage : function (oContext, sx, sy, sw, sh, dx, dy, dw, dh) {
		...
		// if (!isNaN(nImageWidth) && !isNaN(nImageHeight)) {
			// if (sx >= nImageWidth) {
				// sx = nImageWidth - 1;
			// }
			// if (sy >= nImageHeight) {
...
// 				
	...
	draw : function (nFrameDuration, nX, nY, nLayerWidth, nLayerHeight, oContext) {
		...
		if (this._bUseCache || htInfo.scaleX != 1 || htInfo.scaleY != 1 || htInfo.angle !== 0 || htInfo.opacity != 1) {
			bUseTransform = true;
			oContext.translate(nX + nOriginX, nY + nOriginY);
		
			if (htInfo.opacity != 1) {
				nSavedOpacity = oContext.globalAlpha;
				oContext.globalAlpha = oContext.globalAlpha === 0 ? htInfo.opacity : oContext.globalAlpha * htInfo.opacity;
			}
		
			if (htInfo.angle !== 0) {
				oContext.rotate(collie.util.toRad(htInfo.angle));
			}
			
			if (htInfo.scaleX !== 1 || htInfo.scaleY !== 1) {
				oContext.scale(htInfo.scaleX, htInfo.scaleY);
			}
			
			oContext.translate(-nOriginX, -nOriginY);
			nX = nY = 0;
		}
		...
		if (!this._bUseCache || !this._oDisplayObject.isChanged(true)) {
			// 캐시 그리기 전에 비워줌
			this.clearCache();
			...
			/**
			 * @name collie.DisplayObject#canvasDraw
			 * @event
			 * @param {Object} htEvent
			 ...
			 * @param {Number} htEvent.y 상대 y좌표
			 */
			this._oDisplayObject.fireEvent("canvasDraw", this._htEvent);
		...
		if (this._oDisplayObject.hasChild() && (!htInfo.useCache || !this._oDisplayObject.isChanged(true))) {
		...
		if (bUseTransform) {
			...
			if (htInfo.opacity !== 1) {
				oContext.globalAlpha = nSavedOpacity;
			}
			...
			if (htInfo.scaleX !== 1 || htInfo.scaleY !== 1) {
				oContext.scale(1 / htInfo.scaleX, 1 / htInfo.scaleY);
			}
			
			oContext.translate(-(nSavedXRatio + nOriginX), -(nSavedYRatio + nOriginY));
		}
	}
});
/**
...
collie.DisplayObjectDOM = collie.Class(/** @lends collie.DisplayObjectDOM.prototype */{
	...
	draw : function (nFrameDuration, nX, nY, nLayerWidth, nLayerHeight) {
		...
		if (htDirty.visibility) {
			this._elContainerStyle.visibility = htInfo.visibility;
		}
		...
		/**
		 * @name collie.DisplayObject#domDraw
		 * @event
		 * @param {Object} htEvent
		 * @param {collie.DisplayObject} htEvent.displayObject
		 ...
		 */
		this._oDisplayObject.fireEvent("domDraw", this._htEvent);
	},
	...
	_makeTranslate : function (nX, nY, nZ) {
		var sTranslate = '';
		var bUseCSS3d = this._htInfo.angle && this._bIsAndroid ? false : this._bSupportCSS3d;
		// getAngle
 ...
 * @param {Number} [htOption.right=0] x축의 부모를 기준으로 우측부터의 위치, x값이 직접 변경되면 값이 일치하지 않을 수 있다. 초기 위치를 잡을 때만 사용해야 함
 * @param {Number} [htOption.bottom=0] y축의 부모를 기준으로 하단부터의 위치, y값이 직접 변경되면 값이 일치하지 않을 수 있다. 초기 위치를 잡을 때만 사용해야 함
 ...
 * @param {String} [htOption.visibility="visible"] 화면 표시 여부. "hidden" 값으로 설정하면 자식 객체는 표시하고 자신만 보이지 않게 한다. 이 상태에서 이벤트는 받을 수 있다.
 */
collie.DisplayObject = collie.Class(/** @lends collie.DisplayObject.prototype */{
	 ...
	 * @private
	 */
	_aPropertyPriority : ['name', 'backgroundImage'],
	
	/**
	 * @constructs
	 */
	$init : function (htOption) {
		// 여기 정의된 옵션이 아니라면 set할 때 changed상태로 변경되지 않는다
		this.htOptions = null;
		this.addOptions({
			name : "", // 객체 이름
			width : 'auto',
	...
	/**
	 * 기본 옵션에 추가한다
	 * 주로 DisplayObject를 상속받아서 처리하는 경우 사용
	 * @private
	 * @param {Object} htOptions
	 */
	addOptions : function (htOptions) {
		if (this.htOptions === null) {
			this.htOptions = collie.util.cloneObject(htOptions);
		} else {
			for (var i in htOptions) {
				this.htOptions[i] = htOptions[i];
			}
		}
	...
	set : function (vKey, vValue, bSkipSetter, bSkipChanged) {
		if (typeof vKey == "object") {
			// 우선순위 있는 값을 먼저 실행
			for (var i = 0, len = this._aPropertyPriority.length; i < len; i++) {
				if (this._aPropertyPriority[i] in vKey) {
					this.set(this._aPropertyPriority[i], vKey[this._aPropertyPriority[i]]);
					delete vKey[this._aPropertyPriority[i]];
				}
			}
			...
			this._htInfo[vKey] = vValue;
			this.setDirty(vKey); // record value to find  
			
			// perform setter and setChanged to properties that defined by addOption addOptions
			if (vKey in this.htOptions) {
				if (!bSkipSetter) {
	...
	_setter : function (vKey, vValue) {
		...
		if (vKey == "zIndex") {
			if (this.getParent()) {
				this.getParent().changeDisplayObjectZIndex(this);
		...
		// 값 보정
		if (vKey == "x" || vKey == "y") {
			// prevent floating position
			// this._htInfo[vKey] = Math.round(this._htInfo[vKey]);
		...
		// 우측, 하단 기준으로 포지션 적용
		if ((vKey == "right" || vKey == "bottom") && vValue !== null) {
			this.alignPosition(vKey == "right" ? vValue : false, vKey == "bottom" ? vValue : false);
		}
		...
		// 캔버스 캐시 생성
		if (vKey == 'useCache' && this._oDrawing !== null) {
			if (vValue) {
	...
	setChanged : function (bChangedTransforms) {
		...
		// 부모가 있다면 부모도 바뀐 상태로 변경
		var oParent = this.getParent();
		
		while (oParent) {
			oParent.setChanged(); // transforms만 바꼈어도 부모에게는 전체가 바뀐것으로 통보
	...
	setLayer : function (oLayer) {
		...
		if (htInfo.right !== null || htInfo.bottom !== null) {
			this.alignPosition(htInfo.right !== null ? htInfo.right : false, htInfo.bottom !== null ? htInfo.bottom : false);
		}
	...
	getRelatedPosition : function () {
		...
		while (oSelf.getParent()) {
			oSelf = oSelf.getParent();
	...
	/**
	 * 객체의 위치를 우측, 하단 기준으로 정한다.
	 * 
	 * @param {Number|Boolean} nRight 우측 기준 x좌표, false면 정렬하지 않음
	 * @param {Number|Boolean} nBottom 하단 기준 y좌표, false면 정렬하지 않음
	 */
	alignPosition : function (nRight, nBottom) {
		var htInfo = this.get();
		var nWidth = 0;
		var nHeight = 0;
		var nX = 0;
		...
		// 기준 크기 구함
		if (this.getParent()) {
			nWidth = this.getParent().get("width");
			nHeight = this.getParent().get("height");
		} else if (this.getLayer()) {
			nWidth = this.getLayer().option("width");
			nHeight = this.getLayer().option("height");
		}
		...
		if (nWidth && nHeight) {
			if (nRight !== false) {
				nX = nWidth - (htInfo.width + nRight);
				this.set("x", nX); 
			}
			
			if (nBottom !== false) {
				nY = nHeight - (htInfo.height + nBottom);
				this.set("y", nY);
			}
		}
	},
	...
	move : function (nX, nY, nVelocity, fCallback) {
		var nCurrentX = this._htInfo.x;
		var nCurrentY = this._htInfo.y;
 ...
 * @param {Number} [htOption.velocityX=0] x축 속도
 * @param {Number} [htOption.velocityY=0] y축 속도
 * @param {Number} [htOption.velocityRotate=0] 회전 속도
 * @param {Number} [htOption.forceX=0] x축 힘
 * @param {Number} [htOption.forceY=0] y축 힘
 * @param {Number} [htOption.forceRotate=0] 회전 힘
...
collie.MovableObject = collie.Class(/** @lends collie.MovableObject.prototype */{
	...
	update : function (nFrameDuration, nX, nY, nLayerWidth, nLayerHeight) {
		var nFrame = Math.max(1, Math.floor(nFrameDuration / collie.Renderer.getDuration()));
	...
	_applyRotation : function (nFrame) {
		if (this._htInfo.forceRotate !== 0) {
			this.set("velocityRotate", this._htInfo.velocityRotate + this._htInfo.forceRotate);
		}
		
		if (this._htInfo.velocityRotate !== 0) {
			var nAngleRad = collie.util.fixAngle(collie.util.toRad(this._htInfo.angle + this._htInfo.velocityRotate * nFrame));
	 ...
	 */
	toString : function () {
		return "MovableObject" + (this._htInfo.name ? " " + this._htInfo.name : "")+ " #" + this.getId() + (this.getImage() ? "(image:" + this.getImage().src + ")" : "");
	}
...
collie.Rectangle = collie.Class(/** @lends collie.Rectangle.prototype */{
	...
	$init : function (htOption) {
		...
		this.option(htOption || {});
		this.set(this.option());
		this.attach("canvasDraw", this._onDrawCanvas.bind(this));
		this.attach("domDraw", this._onDrawDOM.bind(this));
		this._sBorderRadius = collie.util.getCSSPrefix("border-radius", true);
	},
	
	_onDrawDOM : function (oEvent) {
		if (this._bChanged) {
			var htInfo = this.get();
	...
	_onDrawCanvas : function (oEvent) {
		var htInfo = this.get();		
		var oContext = oEvent.context;
		var nRadius = this.get("radius");
...
collie.Circle = collie.Class(/** @lends collie.Circle.prototype */{
	 ...
	 */
	toString : function () {
		return "Circle" + (this.get("name") ? " " + this.get("name") : "")+ " #" + this.getId() + (this.getImage() ? "(image:" + this.getImage().src + ")" : "");
	}
}, collie.DisplayObject);
/**
...
collie.Text = collie.Class(/** @lends collie.Text.prototype */{
	...
	_onDrawCanvas : function (oEvent) {
		...
		this._oContext.fillStyle = this._htInfo.fontColor;
	...
	_onDrawDOM : function (oEvent) {
		...
		elStyle.color = this._htInfo.fontColor;
		...
		elStyle.textAlign = this._htInfo.textAlign;
	...
	_getFontText : function () {
		return this._htInfo.fontWeight + " " + (this._htInfo.fontSize * this._nRatio) + "px " + this._htInfo.fontFamily;
	},
	
	_getLineHeight : function () {
		return this._htInfo.lineHeight == "auto" ? (this._htInfo.fontSize * this._nRatio) : this._htInfo.lineHeight * this._nRatio;
	},
	...
	_fillTextMultiline : function (aText, nX, nY) {
		...
		var nMaxLine = this._htInfo.ellipsisMaxLine;
		this._nTextWidth = 0;
		...
		for (var i = 0; i < aText.length; i++) {
			...
			if (this._htInfo.textAlign == "center") {
				nLeft = this._getMaxWidth() / 2 - nTextWidth / 2 + this._getPadding("left");
			} else if (this._htInfo.textAlign == "right") {
				nLeft = ((this._htInfo.width * this._nRatio) - this._getPadding("right")) - nTextWidth;
			}
...
collie.Animation = collie.Class(/** @lends collie.Animation.prototype */{
	...
	/**
	 * 정지
	 * 정지만 해서는 타이머가 제거되지 않는다. remove를 해야 한다.
	 * @see collie.Animation#remove
	 */
	stop : function () {
		if (this.isPlaying()) {
			this._bIsPlaying = false;
 ...
 * @param {Function} [htOption.effect=collie.Effect.linear] 효과 함수
...
collie.AnimationTransition = collie.Class(/** @lends collie.AnimationTransition.prototype */{
	...
	$init : function (fCallback, nDuration, htOption) {
		this.option({
			from : 0, // 시작 값(배열로 넣을 수 있음)
			to : 0, // 끝 값(배열로 넣을 수 있음)
			set : "",
			loop : 1,
		...
		if (!("from" in htOption) && typeof fCallback != "function") {
	...
	run : function (nCurrentFrame, nFrameDuration) {
		...
		this.triggerCallback({
			...
			cycle : this._nCountCycle,
			runningTime : this._nRunningTime,
			from : this._htOption.from,
			to : this._htOption.to,
			value : this._nValue // 값이 배열이면 이것도 배열로 반환됨
		});
		...
		if (this._nRunningTime >= this._nDuration) {
			...
			this._transitionValue(this._nRunningTime - this._nRunLastestTime);
		}
	...
	_isEndValue : function () {
		if (this._bIsArrayValue) {
			for (var i = 0, len = this._htOption.to.length; i < len; i++) {
				if (this._nValue[i] !== this._getEndValue(this._nValue[i], this._htOption.from[i], this._htOption.to[i])) {
					return false;
				}
			}
			...
			return this._nValue === this._getEndValue(this._nValue, this._htOption.to, this._htOption.from);
		}
	},
	...
	_setEndValue : function () {
		if (this._bIsArrayValue) {
			for (var i = 0, len = this._htOption.to.length; i < len; i++) {
				this._nValue[i] = this._getEndValue(this._nValue[i], this._htOption.from[i], this._htOption.to[i]);
			}
		} else {
			this._nValue = this._getEndValue(this._nValue, this._htOption.to, this._htOption.from);
		}
	},
	
	/**
	 * 값 두개를 넣어 현재 값과 가까운 값을 반환 한다
	 * 첫 값과 끝 값이 바뀌어도 관계 없다.
	 * @private
	 * @param {Number} nValue 현재 값
	 * @param {Number} nValue1 첫 값
	 * @param {Number} nValue2 끝 값
	 * @return {Number} nValue와 가까운 값을 반환
	 */
	_getEndValue : function (nValue, nValue1, nValue2) {
		var nValueMin = Math.min(Math.abs(nValue - nValue1), Math.abs(nValue - nValue2));
		return (nValueMin == Math.abs(nValue - nValue1)) ? nValue1 : nValue2;
	}
}, collie.Animation);
/**
 ...
 * @param {Number} [htOption.start] 시작값이 아닌 값부터 시작할 경우 값을 설정
 * @example
...
collie.AnimationCycle = collie.Class(/** @lends collie.AnimationCycle.prototype */{
	...
	run : function (nCurrentFrame, nFrameDuration) {
		...
		if (this._nRunningTime === 0 || this._nRunLastestTime + this._nTerm <= this._nRunningTime) {
			if (htOption.to > htOption.from ? this._nValue >= htOption.to : this._nValue <= htOption.to) {
				this._nValue = this.option("from");
			} else {
				this._nValue += (htOption.step * (1 + nSkippedCount));
				
				// 지나쳤으면 처리
				if (htOption.to > htOption.from ? this._nValue > htOption.to : this._nValue < htOption.to) {
					if (!htOption.loop || htOption.loop - 1 > this._nCountCycle) {
						this._nValue = htOption.from + ((this._nValue - htOption.to) % (nDiff + 1));
					} else {
						this._nValue = htOption.to;
					}
					
					this._nCountCycle += Math.max(0, Math.floor((this._nValue - htOption.to) / (nDiff + 1))) + 1;
				}
			}
...
/**
 * 계획된 여러 애니메이션을 다룰 수 있는 Queue<br /><br />
 * 
 * @see collie.Timer
 * @class collie.AnimationQueue
 * @extends collie.Animation
 * @param {Array} aQueue 애니메이션 배열
 * @param {Object} [htOption]
...
collie.AnimationQueue = collie.Class(/** @lends collie.AnimationQueue.prototype */{
	 ...
	 */
	add : function (vType, fCallback, nDuration, htOption) {
		if (vType instanceof Array) {
			for (var i = 0, len = vType.length; i < len; i++) {
				this.add.apply(this, vType[i]);
			}
		} else {
			var oAnimation;
	...
	next : function () {
		...
		this._aAnimations[this._nAnimationIdx].reset();
...
collie.TimerList = collie.Class(/** @lends collie.TimerList.prototype */{
	...
	/**
	 * 애니메이션 추가
	 * TODO TimerList와 Animation 들의 의존성 제거 생각해 봐야 함
	 * 
	 * @param {collie.Animation} oAnimation
	 */
	add : function (oAnimation) {
		var nLength = this._aList.push(oAnimation);
		this._htListIdx[oAnimation.getId()] = nLength - 1;
		oAnimation.setTimerList(this);
	},
	
	_makeIdxHash : function () {
		this._htListIdx = {};
		
		for (var i = 0, len = this._aList.length; i < len; i++) {
			this._htListIdx[this._aList[i].getId()] = i;
		}
	},
	 ...
	 * 애니메이션 제거(멈춤이라고 보면 됨)
	 * 
	 * @param {collie.Animation|Number} vAnimation 제거할 애니메이션 아이디, 인스턴스 
	 */
	remove : function (vAnimation) {
		var nId = (typeof vAnimation == 'object') ? vAnimation.getId() : vAnimation;
		var nIdx;
		
		if (nId in this._htListIdx) {
			nIdx = this._htListIdx[nId];
			this._aList.splice(nIdx, 1);
			this._makeIdxHash();
		}
	},
	...
	run : function (nCurrentFrame, nFrameDuration) {
		for (var i = 0, len = this._aList.length; i < len; i++) {
			if (this._aList[i] && this._aList[i].isPlaying()) {
...
collie.Renderer = new (collie.Class(/** @lends collie.Renderer */{
	 ...
	 */
	_getNameAnimationFrame : function () {
		var aNames = [
	 ...
	 * 
	 * @param {Number} nDuration 1프레임 시간 간격
	 ...
	 */
	start : function (nDuration, fCallback) {
		if (!this._bPlaying) {
			// this.stop();
			this._nDuration = nDuration || this.RENDERING_DURATION; // 기본 16ms
			this._fCallback = fCallback || null;
			this._bPlaying = true;
			
			// FPS 60보다 낮으면 requestAnimationFrame 사용하지 않음
	...
	_trigger : function (nDelay) {
		...
		if (this._sRequestAnimationFrameName !== false && !this.DEBUG_USE_DELAY) {
			window[this._sRequestAnimationFrameName](this._fRender);
		} else {
